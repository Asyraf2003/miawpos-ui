...
Responsive browser client for the authoritative sibling `meawpos-api`. The R6 authentication slice uses Google OIDC for production human login, then recovers the authoritative MiawPOS session after reload and revokes it on logout. Operational screens are outside this slice.

Canonical rules: [AGENTS.md](AGENTS.md), then the backend's docs chain. Implementation and verification: [R6 session proof](docs/r6-session-proof.md).
...
- `presentation/` and `components/ui/`: Google login action, outcome feedback, responsive shell, owned shadcn source.
...
Local HTTPS simulation passed; a deployed production host, CI/history release checks, and representative physical-tablet validation remain later R6 proof obligations. This login slice does not close the full R6 MVP.
