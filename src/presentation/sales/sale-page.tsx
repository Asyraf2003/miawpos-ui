...
export function SalePage({ rootId }: { rootId: string }) {
  ...
  return <section className="space-y-5 border-t pt-6">
    ...
    {sale.isFetching ? <p role="status">{t('sale.loading')}</p> : sale.isError ? <><OutcomeFeedback value={outcomeFrom(sale.error)} /><Button onClick={() => { void sale.refetch() }}>{t('action.retry')}</Button></> : sale.data ? <>
      ...
      <ul className="divide-y">{sale.data.lines.map((line, index) => <li key={index} className="flex flex-wrap justify-between gap-3 py-3"><span className="break-words">{line.name} × {line.quantity}</span><span className="tabular-nums">{money(line.totalRupiah)}</span></li>)}</ul>
      <dl className="grid grid-cols-2 gap-3 border-t pt-4 tabular-nums"><dt>{t('sale.total')}</dt><dd className="text-right font-semibold">{money(sale.data.totalRupiah)}</dd><dt>{t('sale.tender')}</dt><dd className="text-right">{money(sale.data.tenderedRupiah)}</dd><dt>{t('sale.change')}</dt><dd className="text-right">{money(sale.data.changeRupiah)}</dd></dl>
      {sale.data.reversal ? <div className="space-y-2 border-t pt-4"><p>{t('sale.refund')}: <strong className="tabular-nums">{money(sale.data.reversal.refundRupiah)}</strong></p><p className="break-words">{sale.data.reversal.reason}</p></div> : <form className="space-y-4 border-t pt-6" onSubmit={event => { void reverse(event) }}>
