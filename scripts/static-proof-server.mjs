...
const server = https.createServer({ key: readFileSync(process.env.PROOF_TLS_KEY), cert: readFileSync(process.env.PROOF_TLS_CERT) }, (req, res) => {
  ...
  if (req.url === '/api' || req.url?.startsWith('/api/')) {
    const upstream = http.request({ hostname: '127.0.0.1', port: Number(process.env.PROOF_API_PORT ?? 8081), path: req.url, method: req.method,
...
server.listen(4173, '127.0.0.1', () => {
  if (process.env.PROOF_READY_DIR) writeFileSync(resolve(process.env.PROOF_READY_DIR, 'host.ready'), '')
  console.log('Static HTTPS proof host ready on https://localhost:4173')
})
